...
// CSV paths
const SCORES_CSV = path.join(__dirname, '..', 'csv-pull', 'market-data', 'data', 'scores', 'confidence_scores_20251231.csv');
const PRICE_CSVS = {
  equity: path.join(__dirname, '..', 'csv-pull', 'market-data', 'data', 'unintegrated', 'equities', 'equities_solstice_equinox.csv'),
  commodities: path.join(__dirname, '..', 'csv-pull', 'market-data', 'data', 'unintegrated', 'commodities', 'commodities_solstice_equinox.csv'),
  crypto: path.join(__dirname, '..', 'csv-pull', 'market-data', 'data', 'unintegrated', 'crypto', 'crypto_solstice_equinox.csv'),
  forex: path.join(__dirname, '..', 'csv-pull', 'market-data', 'data', 'unintegrated', 'forex', 'forex_solstice_equinox.csv'),
};
...
async function populateFeaturedTickers() {
  ...
  for (const [category, symbols] of Object.entries(CATEGORY_MAP)) {
    ...
    for (let i = 0; i < categoryScores.length; i++) {
      ...
      console.log(`  ✓ ${score.symbol}: $${priceData.price.toFixed(2)} | Score: ${score.total_score}`);
    }
  }
