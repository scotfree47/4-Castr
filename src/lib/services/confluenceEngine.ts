...
/**
 * Calculate comprehensive rating for a single ticker
 * This is the UNIFIED logic combining both engines
 */
export async function calculateTickerRating(
  ...
  try {
    ...
    // 5. Flatten all levels for confluence detection
    const allLevels = [
    ...
    // 7. Calculate all score components

    // Confluence: how many indicators agree on this level
      ...
      favorability:
        seasonalScore >= 80
          ...
          : seasonalScore >= 65
            ...
            : seasonalScore >= 45
              ? "neutral"
              : ("unfavorable" as "very_favorable" | "favorable" | "neutral" | "unfavorable"),
    }

    // 15. Return unified rating
    return {
      symbol,
      category,
...
// ============================================================================
// FEATURED TICKER SELECTION
// ============================================================================

/**
 * Calculate featured tickers for all categories
 * Excludes sentinels, filters for 85%+ confidence
 */
export async function calculateAllFeaturedTickers(): Promise<{
  ...
  for (const [category, symbols] of Object.entries(ALL_TICKERS)) {
    ...
    const nonSentinels = symbols.filter((s) => !SENTINELS.has(s))
    ...
    const ratings = await batchCalculateRatings({
      ...
      minScore: 75, // ✅ CHANGED: 75% confidence minimum (was 85%)
      maxResults: 10,
      parallelism: 5,
    })
...
/**
 * Store featured tickers to database
 */
export async function storeFeaturedTickers(featured: TickerRating[]): Promise<void> {
  ...
  try {
    ...
    // Delete existing entries for these categories
...
/**
 * Check if refresh needed (solar ingress or time elapsed)
 */
