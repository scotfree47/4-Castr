...
import { TrendingUp, TrendingDown, DollarSign, Activity, Coins, TrendingUpDown, Bitcoin, AlertTriangle, Target } from "lucide-react"
...
export function SentinelsOverview() {
  ...
  const processGroupData = async (group: any, data: any, ingress: any, yesterday: string) => {
    ...
    const sentinelMetrics = await Promise.all(
      sentinels.map(async (symbol: string) => {
        ...
        // 🔥 NEW: Fetch key levels for this symbol
        let nearestSupport, nearestResistance;
        try {
          const levelsRes = await fetch(
            `/api/levels/${symbol}?startDate=${new Date(Date.now() - 30*24*60*60*1000).toISOString().split('T')[0]}&endDate=${yesterday}`
          );
          ...
          if (levelsData.success) {
            ...
            current.supportResistance?.forEach((sr: any) => 
              allLevels.push({ price: sr.price, type: sr.type })
            );
            
            const supports = allLevels
              .filter(l => l.price < latestPrice && l.type === 'support')
              .sort((a, b) => b.price - a.price);
            nearestSupport = supports[0]?.price;
            
            const resistances = allLevels
              .filter(l => l.price > latestPrice && l.type === 'resistance')
              .sort((a, b) => a.price - b.price);
            nearestResistance = resistances[0]?.price;
          }
        } catch (err) {
          console.log(`Could not fetch levels for ${symbol}`);
        }
    ...
    return {
      ...
      nearestSupport: strongest.nearestSupport,
      nearestResistance: strongest.nearestResistance,
                ...
                <CardHeader className="relative flex-1">
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      ...
                      {/* 🔥 NEW: Show support/resistance */}
                      {metric.nearestSupport && metric.nearestResistance && (
                        <div className="mt-2 flex items-center gap-3 text-xs">
                          <div className="flex items-center gap-1">
                            <Target className="h-3 w-3 text-green-600" />
                            <span className="text-green-600">S: ${metric.nearestSupport.toFixed(2)}</span>
                          </div>
                          <div className="flex items-center gap-1">
                            <Target className="h-3 w-3 text-red-600" />
                            <span className="text-red-600">R: ${metric.nearestResistance.toFixed(2)}</span>
                          </div>
                        </div>
                      )}
                    </div>
